...
func TestScrapper_store(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			scrapper := NewScrapper(nil, nil, test.pgClient, true, nil, noop.NewTracerProvider().Tracer("test"), nil, nil, false)
...
func TestScrapper_process(t *testing.T) {
	...
	scrapper := NewScrapper(ghClient, gpClient, pgClient, true, srcs, noop.NewTracerProvider().Tracer("test"), []string{"topic:traefik-plugin language:Go archived:false is:public"}, []string{"is:open is:issue is:public author:traefiker"}, false)
